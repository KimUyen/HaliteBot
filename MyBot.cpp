...
int main() {
    ...
    for (;;) {
        ...
        for (const hlt::Ship& ship : map.ships.at(player_id)) {
			...
			int nearestPlanet = map.FindNearestPlanetCanDockFrom(ship);
			//std::cout << "nearestPlanet: " << nearestPlanet << std::endl;
			
			if (nearestPlanet != -1)
			{
				moves.push_back(hlt::Move::dock(ship.entity_id, nearestPlanet));
                    continue;
			}
